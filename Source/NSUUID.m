...
@implementation	NSUUID (Packed)

- (instancetype) initPacked: (uint64_t)value
{
  gsuuid_t      localUUID;

  if (NO == [NSData randomBytes: &localUUID[8] ofLength: UUIDByteCount - 8])
    {
      DESTROY(self);
      return nil;
    }

  localUUID[0] = (uint8_t)(value >> 56);
  localUUID[1] = (uint8_t)(value >> 48);
  localUUID[2] = (uint8_t)(value >> 40);
  localUUID[3] = (uint8_t)(value >> 32);
  localUUID[4] = (uint8_t)(value >> 24);
  localUUID[5] = (uint8_t)(value >> 16);

  /* Version 4 in upper nibble, value bits 15..12 in lower nibble */
  localUUID[6] = 0x40 | ((value >> 12) & 0x0F);

  /* Value bits 11..4 */
  localUUID[7] = (uint8_t)(value >> 4);

  /*
   * RFC variant in upper two bits (10),
   * preserve bits 5..4 (random),
   * value bits 3..0 in lower nibble.
   */
  localUUID[8] = (localUUID[8] & 0x30) | 0x80 | (value & 0x0F);
...
{
  return ((uint64_t)uuid[0] << 56)
    | ((uint64_t)uuid[1] << 48)
    | ((uint64_t)uuid[2] << 40)
    | ((uint64_t)uuid[3] << 32)
    | ((uint64_t)uuid[4] << 24)
    | ((uint64_t)uuid[5] << 16)
    | ((uint64_t)(uuid[6] & 0x0F) << 12)
    | ((uint64_t)uuid[7] << 4)
    | ((uint64_t)(uuid[8] & 0x0F));
}
@end
