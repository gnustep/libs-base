...
- (void) addOperation: (NSOperation *)op
{
  ...
  if (NSNotFound == [internal->operations indexOfObjectIdenticalTo: op]
      && NO == [op isFinished])
  {
    [op addObserver: self forKeyPath: @"isReady"
            options: NSKeyValueObservingOptionNew
            context: NULL];
...
>>>>>>> 35c1962aa (Fix KVO issue when observation removed during observeValueForKepPath notification causing freed heap overwrites)
  [internal->lock lock];
