...
#if defined(__linux__) && !defined(_WIN32) && defined(__GLIBC__) && (__GLIBC__ > 2 || (__GLIBC__ == 2 && __GLIBC_MINOR__ > 28))
...
- (BOOL) launchAndReturnError: (NSError **)error
{
   ...
   */
  {
     ...
     * It's reliably supported in glibc 2.35+ (2022).
     */
...
#if defined(__GLIBC__) && (__GLIBC__ > 2 || (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 35))
    /* If POSIX_SPAWN_SETSID failed (e.g., in containers), retry without it.
     * Some environments don't allow session creation even though the flag exists.
     */
    if (spawn_result == EINVAL || spawn_result == EPERM)
      {
        short flags_no_setsid = POSIX_SPAWN_SETPGROUP | POSIX_SPAWN_SETSIGDEF;
        posix_spawnattr_setflags(&attr, flags_no_setsid);
        spawn_result = posix_spawn(&pid, executable, &file_actions, &attr,
                                   (char* const*)args, (char* const*)envl);
      }
#endif
