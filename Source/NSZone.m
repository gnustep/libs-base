...
fmalloc (NSZone *zone, size_t size)
{
  ...
  GS_RECURSIVE_MUTEX_LOCK(zptr->lock);
  ...
  if (i < bufsize)
    ...
    /* Get memory from segregate fit allocator. */
    {
      ...
      if (chunkhead == NULL)
        {
          GS_RECURSIVE_MUTEX_UNLOCK(zptr->lock);
          if (zone->name != nil)
            [NSException raise: NSMallocException
...
ffree (NSZone *zone, void *ptr)
{
  ...
  GS_RECURSIVE_MUTEX_LOCK(((ffree_zone*)zone)->lock);
  ...
  GS_RECURSIVE_MUTEX_UNLOCK(((ffree_zone*)zone)->lock);
}
...
frecycle1(NSZone *zone)
{
  ...
  if (zptr->blocks == 0)
    {
      GS_RECURSIVE_MUTEX_DESTROY(zptr->lock);
      return YES;
    }
  return NO;
}
  ...
  if (canFree)
    {
      ...
      GS_RECURSIVE_MUTEX_INIT(zone->lock);
      for (i = 0; i < MAX_SEG; i++)
        {
      ...
      if (zone->blocks == NULL)
        {
          GS_RECURSIVE_MUTEX_DESTROY(zone->lock);
          free(zone);
          [NSException raise: NSMallocException
                      format: @"No memory to create zone"];
        }
