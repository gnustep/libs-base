...
static gs_recursive_mutex_t	  modLock;
...
GSLoadModule(NSString *fileName)
{
  ...
  GS_RECURSIVE_MUTEX_LOCK(modLock);
  ...
  GS_RECURSIVE_MUTEX_UNLOCK(modLock);
...
static	gs_recursive_mutex_t	traceLock;
...
GSPrivateReturnAddresses(NSUInteger **returns)
{
  ...
  GS_RECURSIVE_MUTEX_LOCK(traceLock);
  if (0 == hProcess)
    {
      ...
      if (0 == capture)
	{
	  ...
	  if (0 == hModule)
		...
		(int)GetLastError());
	      GS_RECURSIVE_MUTEX_UNLOCK(traceLock);
	      return 0;
	    }
...
{
#if	defined(_WIN32) && !defined(USE_BFD)
  GS_RECURSIVE_MUTEX_INIT(traceLock);
#endif
#if     defined(USE_BFD)
  GS_RECURSIVE_MUTEX_INIT(modLock);
#endif
}
