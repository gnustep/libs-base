...
#define baseData                                                               \
  ((self->_baseURL == 0) ? 0 : ((parsedURL *) (self->_baseURL->_data)))
 ...
 */
static char *
buildURL(parsedURL *base, parsedURL *rel, BOOL standardize);
static id
clientForHandle(void *data, NSURLHandle *hdl);
static char *
findUp(char *str);
static char *
unescape(const char *from, char *to);
...
buildURL(parsedURL *base, parsedURL *rel, BOOL standardize)
{
  ...
  if (rel->isGeneric == YES || rel->user != 0 || rel->password != 0
      || rel->host != 0 || rel->port != 0)
    {
      *ptr++ = '/';
      *ptr++ = '/';
 ...
 */
static BOOL
legal(const char *str, const char *extras)
{
  const char *mark = "-_.!~*'()";
		...
		path:(NSString *)aPath
{
  ...
  if ([aScheme isEqualToString:@"file"])
    {
      aPath =
	[aPath stringByAddingPercentEncodingWithAllowedCharacters:fileCharSet];
    }
  else
    {
      aPath =
  ...
  if ([aPath length] > 0)
	...
	{
	  aUrlString =
	    [aUrlString initWithFormat:@"%@://%@%@", aScheme, aHost, aPath];
	}
      else
	{
	  aUrlString =
...
- (id)initWithString:(NSString *)aUrlString relativeToURL:(NSURL *)aBaseUrl
{
  ...
  if ([aUrlString isKindOfClass:[NSString class]] == NO)
    {
      [NSException
	 raise:NSInvalidArgumentException
	format:@"[%@ %@] bad string parameter", NSStringFromClass([self class]),
	       NSStringFromSelector(_cmd)];
    }
  if (aBaseUrl != nil && [aBaseUrl isKindOfClass:[NSURL class]] == NO)
    {
      [NSException
	 raise:NSInvalidArgumentException
  ...
  NS_DURING
  {
    ...
    if (buf->scheme != 0 && base != 0 && 0 != strcmp(buf->scheme, base->scheme))
      {
		...
		if (legal(buf->user, ";:&=+$,") == NO)
		  {
		    [NSException
		      ...
		      format:@"[%@ %@](%@, %@) "
			     ...
			     NSStringFromSelector(_cmd), aUrlString, aBaseUrl];
		  }
		ptr = strchr(buf->user, ':');
		if (ptr != 0)
		  {
	...
	else if (base != 0 && buf->user == 0 && buf->password == 0
  ...
  if ((errorStr != nil) && (error != NULL))
    {
      ...
      info = [NSDictionary
	dictionaryWithObjectsAndKeys:errorStr, NSLocalizedDescriptionKey, nil];
      *error = [NSError errorWithDomain:@"NSURLError" code:0 userInfo:info];
    }
  ...
  if (shouldUseCache == YES && (d = [handle availableResourceData]) != nil)
    {
       ...
       */
      if ([client respondsToSelector:@selector(URL:
      ...
      if (client != nil)
	{
	  ...
	  NSMapInsert((NSMapTable *) _clients, (void *) handle,
		      (void *) client);
	  [clientsLock unlock];
...
{
  return
    [self _URLBySettingPath:[[self path]
...
{
  return
    [self _URLBySettingPath:[[self path] stringByDeletingLastPathComponent]];
}
  ...
  resourceDidFailLoadingWithReason:(NSString *)reason
{
  ...
  if (c != nil)
    {
      if ([c respondsToSelector:@selector(URL:
...
- (instancetype)initWithName:(NSString *)name value:(NSString *)value
{
  ...
  if (self != nil)
	...
	{
	  ASSIGN(internal->_name, @""); // OSX behaviour is to set an empty
					// string for nil name property
	}
...
{
  // OSX behavior is to return nil for a string which cannot be used to
  // initialize valid NSURL object
  ...
  if (internal->_queryItems != nil)
    {
      ...
      items =
	[[NSMutableArray alloc] initWithCapacity:[internal->_queryItems count]];
