...
/* Legacy Key Value Observing implementation for the GCC runtime. */
...
- (id)initWithClass:(Class)aClass
{
  ...
  if ([aClass instanceMethodForSelector:@selector(takeValue:forKeyPath:)] !=
    ...
    {
      NSLog(
        @"WARNING The class '%@' (or one of its superclasses) overrides"
...
- (void)setterLongLong:(unsigned long long)val
{
  ...
  imp = (void (*)(id, SEL,
                  unsigned long long)) [c instanceMethodForSelector:_cmd];
...
- (GSKVOPathInfo *)lockReturningPathInfoForKey:(NSString *)key
{
  ...
  [iLock lock];
  pathInfo
    = AUTORELEASE(RETAIN((GSKVOPathInfo *) NSMapGet(paths, (void *) key)));
  if (pathInfo == nil)
    {
      [iLock unlock];
    }
            ...
            context:(void *)aContext
{
  ...
  if ([anObserver respondsToSelector:@selector
...
- (void)keyPathChanged:(id)objectToObserve
{
  ...
  if (child != nil)
  ...
  else
    {
      ...
      change =
        [NSMutableDictionary dictionaryWithObject:[NSNumber numberWithInt:1]
                                           forKey:NSKeyValueChangeKindKey];
            ...
            context:(void *)aContext
{
  ...
  if (dot.location != NSNotFound)
    {
      forwarder =
        [[NSKeyValueObservationForwarder alloc] initWithKeyPath:aPath
                                                       ofObject:self
  ...
  while (i != NSNotFound)
    {
      ...
      [elem removeObserver:anObserver forKeyPath:aPath];
  ...
  while (i != NSNotFound)
    {
      ...
      [elem removeObserver:anObserver forKeyPath:aPath context:context];
      ...
      if (pathInfo->recursion++ == 0)
        {
          ...
          [pathInfo->change
            setValue:[NSNumber numberWithInt:NSKeyValueChangeSetting]
              forKey:NSKeyValueChangeKindKey];
      ...
      if (pathInfo->recursion == 1)
        {
          ...
          [pathInfo->change setValue:[NSNumber numberWithInt:changeKind]
                              forKey:NSKeyValueChangeKindKey];
          [pathInfo->change setValue:indexes forKey:NSKeyValueChangeIndexesKey];
          ...
          if (mutationKind == NSKeyValueUnionSetMutation)
            {
              ...
              [set minusSet:oldSet];
              [pathInfo->change
                setValue:[NSNumber numberWithInt:NSKeyValueChangeInsertion]
                  forKey:NSKeyValueChangeKindKey];
              [pathInfo->change setValue:set forKey:NSKeyValueChangeNewKey];
              RELEASE(set);
            }
                   ...
                   || mutationKind == NSKeyValueIntersectSetMutation)
            {
              [oldSet minusSet:set];
              [pathInfo->change
                setValue:[NSNumber numberWithInt:NSKeyValueChangeRemoval]
                  forKey:NSKeyValueChangeKindKey];
          ...
          else if (mutationKind == NSKeyValueSetSetMutation)
            {
              ...
              [new minusSet:oldSet];
              [pathInfo->change
                setValue:[NSNumber numberWithInt:NSKeyValueChangeReplacement]
                  forKey:NSKeyValueChangeKindKey];
...
{
  NSString *selString =
    [NSString stringWithFormat:@"keyPathsForValuesAffecting%@",
