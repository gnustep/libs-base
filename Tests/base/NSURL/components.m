...
int main()
{
  ...
  PASS_EQUAL(item.name, cyrillicStr,
    "Should return decoded query item name from url")
  PASS_EQUAL(item.value, cyrillicStr,
    "Should return decoded query item value from url")
  
  item = [NSURLQueryItem queryItemWithName: cyrillicStr value: cyrillicStr];
  [components setQueryItems: [NSArray arrayWithObject: item]];
  PASS_EQUAL([components string], urlString,
    "Encoded url string from unencoded item")
  PASS_EQUAL([components URL], [NSURL URLWithString: urlString],
    "Encoded url query part from unencoded item")
